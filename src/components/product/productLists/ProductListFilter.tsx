...
export default function ProductListFilter({
  ...
  const handleBrandChange = (brand: string) => {
    ...
    updateFilterState({ selectedBrands: newBrands });
  };

  const handlePriceChange = (price: string) => {
    const newPrices = selectedPrices.includes(price)
      ? selectedPrices.filter((p: string) => p != price)
      : [...selectedPrices, price];
    updateFilterState({ selectedPrices: newPrices });
  };
  ...
  const handleColorChange = (color: string) => {
    ...
    updateFilterState({ selectedColors: newColors });
  };

  const handlePromotionChange = () => {
    updateFilterState({ hasPromotion: !hasPromotion });
  };

  const handleSortChange = (sortValue: string) => {
    updateFilterState({ sortBy: sortValue });
  };

  const handleClearBrands = () => {
    updateFilterState({ selectedBrands: [] });
  };

  // handleClearPromotion
  const handleClearPrices = () => {
    updateFilterState({ selectedPrices: [] });
  };

  const handleClearColors = () => {
    updateFilterState({ selectedColors: [] });
  };

  const handleClearPromotion = () => {
    updateFilterState({ hasPromotion: false });
  };
